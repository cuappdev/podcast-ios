...
class DownloadManager: NSObject {
    ...
    /// Maps download url to the task handling the download
    private var downloadedUrls: [URL: URLSessionDownloadTask] = [:]
    ...
    func cancel(episode: EpisodeMO) {
        guard let url = episode.audioURL, let task = downloadedUrls[url] else { return }
...
extension DownloadManager: URLSessionDownloadDelegate {
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        NotificationCenter.default.post(name: .didCompleteDownload, object: nil)
    }
    ...
    func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64,
        ...
        if totalBytesExpectedToWrite > 0 {
            ...
            let progressDict = ["progress": progress]
            // Send download progress notification
            NotificationCenter.default.post(name: .didUpdateDownloadProgress, object: nil, userInfo: progressDict)
        }
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        // Do we need to send user info?
        NotificationCenter.default.post(name: .didFailDownload, object: nil)
    }
